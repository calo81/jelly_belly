...
class ExperimentHandler extends Actor with ActorLogging {
  ...
  val uiQueues = mutable.Map[String, SourceQueue[ExperimentState]]()
  ...
  override def receive = {
    ...
    case ExperimentUpdated(experimentState) =>
      uiQueues.get("get_experiments").map(_.offer(experimentState))
    case GetExperiments(queue) =>
      uiQueues.put("get_experiments", queue)
  }
